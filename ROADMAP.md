...
6. ✅ **Operator sample comparison / selection support — Done 2026-04-27.** Side-by-side comparison page at `/queue/compare/[group_id]` (client-rendered, brand-scoped). Each sibling column shows per-sample image preview (`Post.image_url` → `composited_image.artifact_url` → "No preview" fallback), full content (headline / caption / CTA / banner), status badge, and per-column actions Approve / Reject / Refine / Details — all reusing existing single-post endpoints + modals. The "Sample N/M" chip in queue rows now links to the comparison page when the group has more than one sample (singletons stay plain). Singleton groups redirect to `/queue/[id]`. No bulk actions in MVP — approving one sample does NOT auto-reject siblings; operators handle sibling cleanup explicitly. Backed by a small `sample_group_id` filter added to `GET /api/posts` (Prisma JSONB path filter).
...
- operators can compare and choose between samples ✅ (`/queue/compare/[group_id]` shipped 2026-04-27)
...
4. ✅ **Adhoc Event automation generation flow — Done 2026-04-28.** Server-side orchestrator at `src/lib/automations/adhoc-events/orchestrator.ts` (`runAdhocEventsAutomation({brand_id_filter?, event_id_filter?, lookahead_hours?})`). Eligibility = `Event.status='active'` AND `Event.auto_generate_posts=true` AND `Brand.active=true` AND ≥1 occurrence in `[now, now+24h]`. Generate Now events synthesize one occurrence at `now`. Recurrence events use `generateOccurrences()` filtered to the lookahead window. Dedup = exact-match `Post.findFirst` on `(brand_id, source_type='event', source_id, source_instance_key, platform)` — same identity as the manual route, so manual + automated runs cannot race-condition into duplicates. Status-agnostic. `created_by` = first admin (TEMPORARY shortcut, shared `getAutomationCreator()` helper). `samples_per_slot=1` MVP. Failure-isolated per-event AND per-slot. Verification: admin-only `POST /api/automations/adhoc-events/run` + `npm run automation:adhoc-events [-- <brand_id> | --event=<id> | --lookahead=<hours>]`. Reuses the same `runGeneration()` pipeline as the manual "Generate Drafts" button — no divergent AI path. Deferred (next steps): Cloud Scheduler job for cadence (same shape as Running Promotions); multi-sample per slot from automation; composite index on Post for the dedup query. See `docs/04-automations.md` "Adhoc Event automation flow" for the complete contract.
