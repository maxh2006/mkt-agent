...
4. 🟡 **Image generation pipeline — structured inputs + split rendering** (near-term priority as of 2026-04-23)
   ...
   6. ⏳ **Deterministic text + logo overlay rendering** — spec complete (safe zones + text zones + logo slots are fully described per layout); the rendering engine itself (Satori / sharp / similar) is not yet implemented. Now that `generation_context_json.image_generation` carries the provider artifact alongside `visual_compiled.safe_zone_config`, this is the next missing piece before final composited images can ship.

   7. 🟡 **Image-rendering provider adapter (background only)** — **Boundary shipped 2026-04-27 (stub-only).** New module [`src/lib/ai/image/`](src/lib/ai/image/) (`types.ts` + `client.ts`) mirrors the text-provider boundary. `AI_IMAGE_PROVIDER=stub` is the default; real adapters (`gemini` / `imagen` / `stability`) are recognised values that throw fail-loud until implemented (no silent fallback). Orchestrator wires it after text generation with failure isolation — text drafts still ship if image generation throws. Per-draft `generation_context_json.image_generation` block carries `provider` / `model` / `status` / `artifact_url` / `provider_asset_id` / `width` / `height` / `background_image_prompt` / `negative_prompt` / `error_code` / `error_message` / timestamps / `render_version`. **`Post.image_url` is intentionally untouched** — reserved for the final composited asset the overlay renderer will produce. Real provider implementation lands in a follow-up.
