...
## Automation-driven generation (2026-04-27, extended 2026-04-28)

Phase 5's source-automation flows live under `src/lib/automations/`:
- `src/lib/automations/running-promotions/` — Running Promotions (shipped 2026-04-27)
- `src/lib/automations/adhoc-events/` — Adhoc Events (shipped 2026-04-28)

**The AI boundary is unchanged**: every automation-driven generation
flows through the exact same `runGeneration()` pipeline operator-
triggered generation uses. There is no separate AI path for automations.
Each orchestrator's only job is the glue (eligibility query, source
fetch / occurrence computation, dedup, loop) — once it produces a
`NormalizedGenerationInput`, the rest of the pipeline (prompt builder
+ provider client + visual compiler + image provider + overlay
renderer + GCS storage + queue inserter) is identical to operator-
triggered runs. The boundary discipline holds: AI still only generates
content from structured packets; automations just produce more packets
on a cadence. See `docs/04-automations.md` for each orchestrator's
full contract.
